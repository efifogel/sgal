...
//! \brief destructor.
Arrangement_on_torus_geo::~Arrangement_on_torus_geo()
{
  clear();
  if (m_owned_aos){
    if (m_aos) {
...
void Arrangement_on_torus_geo::init_prototype()
{
  if (s_prototype) return;
  s_prototype =
    new Container_proto(Arrangement_on_torus_base_geo::get_prototype());
}
...
Container_proto* Arrangement_on_torus_geo::get_prototype()
{
  if (!s_prototype) Arrangement_on_torus_geo::init_prototype();
  return s_prototype;
}

//! \brief sets the ellpsoid attributes.
void Arrangement_on_torus_geo::set_attributes(Element* elem)
{
  Arrangement_on_torus_base_geo::set_attributes(elem);
  ...
  for (cai = elem->cont_attrs_begin(); cai != elem->cont_attrs_end(); ++cai) {
    ...
    if (name == "overlay") {
      Shared_arrangement_on_torus_geo aos_geo =
        boost::dynamic_pointer_cast<Arrangement_on_torus_geo>(cont);
...
void Arrangement_on_torus_geo::clean()
{
  m_dirty = false;
  if (!m_aos) {
    m_aos = new Arrangement_on_torus;
...
void Arrangement_on_torus_geo::clear()
{
...
void Arrangement_on_torus_geo::create_renderers()
{
  m_edges_renderer = new Torus_edges_renderer(*this);
  m_vertices_renderer = new Torus_vertices_renderer(*this);
  m_isolated_vertices_renderer = new Torus_isolated_vertices_renderer(*this);
  m_colored_edges_renderer = new Torus_colored_edges_renderer(*this);
  m_colored_vertices_renderer = new Torus_colored_vertices_renderer(*this);
  m_colored_isolated_vertices_renderer =
    new Torus_colored_isolated_vertices_renderer(*this);

  m_line_colored_edges_renderer =
    new Torus_line_colored_edges_renderer(*this);
  m_point_colored_vertices_renderer =
    new Torus_point_colored_vertices_renderer(*this);
  m_ring_colored_vertices_renderer =
    new Torus_ring_colored_vertices_renderer(*this);
  m_point_colored_isolated_vertices_renderer =
    new Torus_point_colored_isolated_vertices_renderer(*this);
  m_ring_colored_isolated_vertices_renderer =
    new Torus_ring_colored_isolated_vertices_renderer(*this);

  m_inflated_line_edges_renderer =
    new Torus_inflated_line_edges_renderer(*this);
  m_inflated_strip_edges_renderer =
    new Torus_inflated_strip_edges_renderer(*this);
  m_inflated_tube_edges_renderer =
    new Torus_inflated_tube_edges_renderer(*this);
}

//! \brief destroys the renderers.
void Arrangement_on_torus_geo::destroy_renderers()
{
...
Arrangement_on_torus* Arrangement_on_torus_geo::get_aos()
{
  if (m_dirty) clean();
  return m_aos;
}
...
void Arrangement_on_torus_geo::set_aos(Arrangement_on_torus* aos)
{
  m_dirty = false;
  m_aos = aos;
}
