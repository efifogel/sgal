...
public:
  ...
  /*! Constructor */
  Lower_envelope_sphere_geo(Boolean proto = SGAL_FALSE);
  ...
  { return new Lower_envelope_sphere_geo(SGAL_TRUE); }

  /*! Clone */
  virtual SGAL::Container * clone() { return new Lower_envelope_sphere_geo(); }
...
private:
  ...
  OutputIterator get_spheres(OutputIterator oi)
  {
    ...
    for (it = m_surfaces.begin(); it != m_surfaces.end(); ++it) {
      ...
      Sphere * sphere = dynamic_cast<Sphere *>(geo);
