 ...
 */
class SGAL_CLASSDEF Polygon_set_on_sphere_geo :
...
public:
  ...
  /*! Constructor */
  Polygon_set_on_sphere_geo(Boolean proto = SGAL_FALSE);
  ...
  { return new Polygon_set_on_sphere_geo(SGAL_TRUE); }
...
protected:
  ...
  */
  bool is_operation_type(Operation_type op) const {
    return ((op >= SET_COMPLEMENT) &&
            (op < NUM_OF_OPERATIONS));
  }
