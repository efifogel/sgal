...
# should be supported: a pain getter and a getter with a const qualified
# return type.
...
#! \param fields
def generate_hpp(class_name, derived_class_name, fields):
  hpp_filename = os.path.join(class_name + ".hpp")
...
def print_set_attributes_definition(out, class_name, derived_class_name, fields):
  ...
  if 0 < len(multi_string_fields):
    ...
    for field in multi_string_fields[:]:
      ...
      print_line(out, "elem->mark_delete(msai);")
      print_line(out, "continue;")
      print_line(out, "}", dec=True)
  ...
  if 0 < len(container_fields):
    ...
    for field in container_fields[:]:
      ...
      type = field['type']
      print_line(out, "if (name == \"%s\") {" % name, inc=True)
      field_class_name = name[7:].capitalize()
      print_line(out, "auto %s = boost::dynamic_pointer_cast<%s>(cont);" % (name, field_class_name))
      print_line(out, "elem->mark_delete(msai);")
      print_line(out, "continue;")
      print_line(out, "}", dec=True)
...
#! \param derived_class_name
#! \param fields
def generate_cpp(class_name, derived_class_name, fields):
  hpp_filename = os.path.join(class_name + ".cpp")
  with open(hpp_filename, 'w') as out:
    print_line(out, copyright_text)
    print_line(out, "// Generated by " + os.path.basename(__file__))
    print_empty_line(out)
    print_cpp_include_directives(out)
    print_empty_line(out)
    print_line(out, "SGAL_BEGIN_NAMESPACE")
    print_empty_line(out)
...
if __name__ == '__main__':
  ...
  with open(fullname, 'r') as ins:
    ...
    generate_cpp(class_name, derived_class_name, fields)
