...
const std::string Epic_coord_array_3d::s_tag = "EpicCoordinate";

//! The node prototype.
Container_proto* Epic_coord_array_3d::s_prototype(nullptr);

//! Register to the container factory.
REGISTER_TO_FACTORY(Epic_coord_array_3d, "Epic_coord_array_3d");

//! \brief constructs.
Epic_coord_array_3d::Epic_coord_array_3d(Boolean proto) :
  Coord_array(proto),
  m_dirty_inexact_coords(true)
{}

//! \brief constructs.
Epic_coord_array_3d::Epic_coord_array_3d(Size n) :
  m_dirty_inexact_coords(true)
{ m_array.resize(n); }

//! \brief initializes the node prototype.
void Epic_coord_array_3d::init_prototype()
{
...
void Epic_coord_array_3d::delete_prototype()
{
  delete s_prototype;
  s_prototype = nullptr;
}
...
Container_proto* Epic_coord_array_3d::get_prototype()
{
  if (!s_prototype) Epic_coord_array_3d::init_prototype();
  return s_prototype;
}

//! Sets the attributes of the object extracted from the VRML or X3D file.
void Epic_coord_array_3d::set_attributes(Element* elem)
{
  m_array.clear();
...
// #endif
        ...
        Epic_FT fy(y);
        Epic_FT fz(z);
        m_array[i] = Epic_point_3(fx,fy,fz);
      }
    ...
    if (name == "polarPoint") {
      ...
      for (auto i = 0; i < size; i++) {
        ...
        Epic_FT ut(utanhalf);
        Epic_FT ut_square = CGAL::square(ut);
        Epic_FT cosu = (1 - ut_square) / (1 + ut_square);
        Epic_FT sinu = 2 * ut / (1 + ut_square);

        float vtanhalf = tanf(v_rad * 0.5f);
        // CGAL::Gmpq ev(static_cast<int>(vtanhalf * 1000), 1000);
        Epic_FT vt(vtanhalf);
        Epic_FT vt_square = CGAL::square(vt);
        Epic_FT cosv = (1 - vt_square) / (1 + vt_square);
        Epic_FT sinv = 2 * vt / (1 + vt_square);

        Epic_FT fx = cosu * cosv;
        Epic_FT fy = cosu * sinv;
        Epic_FT fz = sinu;
#endif
    ...
    if (name == "exactPoint") {
      auto num_values = get_num_values<Epic_FT>(value);
      auto size = num_values / 3;
      m_array.resize(size);
      std::istringstream svalue(value, std::istringstream::in);
      //! svalue.seekg(0); why this doesn't work?
...
Size Epic_coord_array_3d::data_size() const
{ return m_array.size() * sizeof(Vector3f); }

//! \brief obtains the data.
inline const GLfloat* Epic_coord_array_3d::datum(Uint i) const
{
...
// \brief obtains the data.
const GLfloat* Epic_coord_array_3d::data() const
{
  if (m_dirty_inexact_coords) clean_inexact_coords();
...
const std::vector<Vector3f>& Epic_coord_array_3d::get_inexact_coords() const
{
  if (m_dirty_inexact_coords) clean_inexact_coords();
...
const Vector3f& Epic_coord_array_3d::get_inexact_coord(Uint i) const
{
  if (m_dirty_inexact_coords) clean_inexact_coords();
...
void Epic_coord_array_3d::clean_inexact_coords() const
{
...
void Epic_coord_array_3d::clear()
{
  m_array.clear();
