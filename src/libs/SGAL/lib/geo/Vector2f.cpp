...
//!
void Vector2f::scale(Float s, const Vector2f & v)
{
  m_vector[0] = s * v[0];
  m_vector[1] = s * v[1];
}
...
//!
bool Vector2f::equal(const Vector2f &  v) const
{ return (m_vector[0] == v[0] && m_vector[1] == v[1]); }

//!
bool Vector2f::operator==(const Vector2f& v) const { return equal(v); }
...
//! \brief computes the dot product of the vector.
...
//!
void Vector2f::add_scaled(const Vector2f& v1, Float s, const Vector2f& v2)
{
...
Float Vector2f::length() const { return squarerootf(sqr_length()); }
