...
void Vector3f::add_scaled(const Vector3f & v1, float s,
                                 const Vector3f & v2)
{
...
Vector3f::vector3f(const std::string &text)
{
  ...
  if (vals.size() != 3)
    m_vector[0] = m_vector[1] = m_vector[2] = 0.0;
...
float sqr_distance_pt3(Vector3f p1, Vector3f p2)
{
  return ((sgal::square(p1[0] - p2[0]) + sgal::square(p1[1] - p2[1]) + 
           sgal::square(p1[2] - p2[2])));
}
...
/*! Sets this vector to be the inhomogeneous coordinates (that, divided by
 * w) of (v,1) * m where v is treated as a row vector.
 */
...
/*! Sets this vector to be the cross product of v1 and v2.
 */
