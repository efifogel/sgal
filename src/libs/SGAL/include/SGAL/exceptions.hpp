...
#ifndef SGAL_EXCEPTIONS_HPP
#define SGAL_EXCEPTIONS_HPP
...
// [Efi] This is borrowed from SGAL.

// [Sylvain] This was originaly written in the Exacus library.
// I kept most doxygen comments.

/*! \defgroup SGAL_assert Assertions
 * \brief <tt>\#include <SGAL/basic.h></tt> for pre- and postconditions,
 * assertions, warnings, and error handler.
 *
 * \c SGAL/basic.h provides macros for pre- and post-conditions, assertions,
 * warnings, and errors that are active by default. There are variants for
 * expensive checks that are inactive by default.
 *
 * A failed precondition, postcondition or assertion is a fatal error and the
 * library aborts by default the program execution. For warnings the program
 * continues to run. Both can be changed with the respective \c error_behavior
 * or \c warning_behavior, or with the corresponding \c error_handler or
 * \c warning_handler.
 *
 * However, if the checks are disabled with corresponding preprocessor symbols
 * or \c NDEBUG, none of this error handling will happen and the program
 * execution continues with potentially disastrous results. So, these checks
 * are not for normal error handling, for example testing proper user input or
 * file formats. Furthermore, it is recommended to keep the checks also in
 * the production version of the program, keeping in mind that they should not
 * cost too much runtime overhead, maybe 10%.
 *
 * \see \c SGAL_test for checks that cannot be disabled and \c SGAL_error_msg
 * for an unconditional error message that cannot be disabled with macros.
 */
