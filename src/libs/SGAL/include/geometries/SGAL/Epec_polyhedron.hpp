...
struct Epec_halfedge : public CGAL::HalfedgeDS_halfedge_base<Refs> {
  /*! Construct default. */
  Epec_halfedge() : m_creased(false) {}
                                    ...
                                    typename Traits::Plane_3>
{
  Vector3f m_normal;
  Epec_face() {}
  Epec_face(const typename Traits::Plane_3& pln) :
...
struct Epec_polyhedron_items : public CGAL::Polyhedron_items_3 {
  ...
  struct Vertex_wrapper {
    typedef Epec_vertex<Refs, Traits> Vertex;
  };

  template <typename Refs, typename Traits>
  struct Halfedge_wrapper {
    typedef Epec_halfedge<Refs> Halfedge;
  };

  template <typename Refs, typename Traits>
  struct Face_wrapper {
    typedef Epec_face<Refs, Traits> Face;
  };
};
