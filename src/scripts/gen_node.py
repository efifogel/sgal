...
def generate_hpp(config, fields, out):
  ...
  print_line(out, 'class SGAL_FPG_DECL {} : public {} {{'.format(class_name, derived_class_name))
...
def generate_hpp_from_path(config, fields, output_path):
  ...
  with open(hpp_filename, 'w') as out:
    generate_hpp(config, fields, out)
...
#! \param fields
#! \param output_path
def generate_cpp_from_path(config, fields, output_path):
  class_name = config.get('DEFAULT', 'name')
  derived_class_name = config.get('class', 'base')
...
if __name__ == '__main__':
  ...
  with open(fullname, 'r') as confis:
    ...
    with open(fields_fullname, 'r') as ins:
      ...
      generate_hpp_from_path(config, fields, args.output_inc_path)
      generate_cpp_from_path(config, fields, args.output_lib_path)
